...
async function getProductsBySales(req, res) {
  const { id } = req.params;
  if (!Number(id)) {
    res.sendStatus(400);
    return;
  ...
  try {
    const products = await connection.query(`
            SELECT products.*, products_sales.sales_id, products_sales.new_price 
                FROM products 
            JOIN products_sales 
                ON products.id = products_sales.product_id 
            WHERE products_sales.sales_id = $1 ORDER BY id LIMIT 60;
        `, [id]);
