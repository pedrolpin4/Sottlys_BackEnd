...
describe('GET /products-category/:id', () => {
  ...
  beforeAll(async () => {
    ...
    await connection.query("INSERT INTO products (name, description, price, installments) VALUES ('mais', 'blusa super confortavel e suave', 150.00, 6);");

    const category = await connection.query("SELECT * FROM categories WHERE name = 'cat1';");
    const idCategory = category.rows[0].id;
    const product = await connection.query("SELECT * FROM products WHERE name = 'mais';");
  ...
  afterAll(async () => {
    ...
    await connection.query("DELETE FROM products WHERE name = 'mais';");
